...
public class TransferProcessingService {
    ...
    public TransferResult processTransfer(TransferCommand command, BankUserPrincipal principal, String correlationId) {
        ...
        recordAuditEvent(
                new InternalAuditEventRequest(
                        ...
                        "SUCCESS",
                        correlationId,
                        "Transfer " + result.transactionId() + " created for account " + command.fromAccountId()
                ),
                correlationId
        );
    ...
    private void ensureCustomerEligible(BankUserPrincipal principal, InternalCustomerEligibility customer) {
        if ("CUSTOMER".equals(principal.role()) && !"VERIFIED".equals(customer.kycStatus())) {
            throw new ResponseStatusException(FORBIDDEN, "Customer KYC status does not allow transfer initiation");
        }

        if ("CUSTOMER".equals(principal.role()) && "HIGH".equals(customer.riskRating())) {
            throw new ResponseStatusException(FORBIDDEN, "Customer risk level requires manual review before transfer");
        }
    }
    ...
    private InternalCustomerEligibility loadCustomerEligibility(String username, String correlationId) {
        ...
        return switch (response.statusCode()) {
            ...
            case 404 -> throw new ResponseStatusException(NOT_FOUND, "Customer profile not found for transfer");
    ...
    private void recordAuditEvent(InternalAuditEventRequest request, String correlationId) {
        ...
        if (response.statusCode() < 200 || response.statusCode() >= 300) {
            throw new ResponseStatusException(BAD_GATEWAY, "Audit-service could not persist the transfer audit event");
        }
    }
